...
{
    internal static class AdministrationGroupQueriesRequestingProvider
    {
        public static string SearchRequestingProvider(RequestingProviderSearchCriteria criteria, bool count = false)
        {
            if (count)
            {
                return @$"
                    ...
                    WHERE pnl.[LineOfBusinessId] = @LineOfBusinessId AND p.[AdministrationGroupId] = @AdministrationGroupId AND p.RenderingProviderEntityTypeId = {(int)ProviderEntityTypes.Individual}
        ...
        public static string SearchRequestingProviderWithHiearchy(RequestingProviderSearchCriteria criteria, bool count = false)
        {
            if (count)
            {
                return @$"
                    SELECT COUNT(DISTINCT p.[ProviderAffiliationId])
                        FROM [dbo].[ProviderDirectory] p 
                             JOIN [dbo].[ProductNetworkLineOfBusiness] pnl ON pnl.[NetworkId] = p.[NetworkId]
                             JOIN [dbo].[AdministrationGroupHierarchy] a ON a.ChildAdministrationGroupId = p.AdministrationGroupId
                    WHERE pnl.[LineOfBusinessId] = @LineOfBusinessId AND a.[ParentAdministrationGroupId] = @AdministrationGroupId AND p.RenderingProviderEntityTypeId = {(int)ProviderEntityTypes.Individual}
                        {(!string.IsNullOrEmpty(criteria.ProviderName) ? " AND (p.[RenderingProviderName] LIKE @ProviderName OR (p.[BillingProviderName] LIKE @ProviderName AND p.IsMSOClinic = 0) OR (p.[ProviderLocationName] LIKE @ProviderName AND p.IsMSOClinic = 1))" : string.Empty)}
                        {(!string.IsNullOrEmpty(criteria.NPI) ? " AND (p.[RenderingProviderNPI] LIKE @ProviderNPI OR p.[BillingProviderNPI] LIKE @ProviderNPI)" : string.Empty)}
                ";
            }
            ...
            return @$"
                ...
                FROM [dbo].[ProviderDirectory] p 
                     JOIN [dbo].[ProductNetworkLineOfBusiness] pnl ON pnl.[NetworkId] = p.[NetworkId]
                     JOIN [AdministrationGroupHierarchy] a ON a.ChildAdministrationGroupId = p.AdministrationGroupId
                WHERE pnl.[LineOfBusinessId] = @LineOfBusinessId AND a.[ParentAdministrationGroupId] = @AdministrationGroupId AND p.RenderingProviderEntityTypeId = {(int)ProviderEntityTypes.Individual}
                        {(!string.IsNullOrEmpty(criteria.ProviderName) ? " AND (p.[RenderingProviderName] LIKE @ProviderName OR (p.[BillingProviderName] LIKE @ProviderName AND p.IsMSOClinic = 0) OR (p.[ProviderLocationName] LIKE @ProviderName AND p.IsMSOClinic = 1))" : string.Empty)}
                        {(!string.IsNullOrEmpty(criteria.NPI) ? " AND (p.[RenderingProviderNPI] LIKE @ProviderNPI OR p.[BillingProviderNPI] LIKE @ProviderNPI)" : string.Empty)}
                GROUP BY
        ...
        public static string GetProvidersLocationsWithHiearchy()
        {
            return @$"
                SELECT DISTINCT 
                       p.[ProviderAffiliationId]
                ...
                FROM  [dbo].[ProviderDirectory] p
                      JOIN [dbo].[ProductNetworkLineOfBusiness] pnl ON pnl.[NetworkId] = p.[NetworkId]
                      JOIN [AdministrationGroupHierarchy] a ON a.ChildAdministrationGroupId = p.AdministrationGroupId
                WHERE pnl.[LineOfBusinessId] = @LineOfBusinessId AND a.[ParentAdministrationGroupId] = @AdministrationGroupId AND p.ProviderAffiliationId IN @ProviderAffiliationIds
                ORDER BY p.[ProviderAffiliationId], p.[City] 
            ";
        }

        public static string GetProvidersSpecialtiesWithHiearchy()
        {
            return @$"
                SELECT DISTINCT 
                       p.[ProviderAffiliationId]
        ...
        public static string GetProvidersLocations()
        {
            return @$"
                ...
                FROM  [dbo].[ProviderDirectory] p
                      JOIN [dbo].[ProductNetworkLineOfBusiness] pnl ON pnl.[NetworkId] = p.[NetworkId]
                WHERE pnl.[LineOfBusinessId] = @LineOfBusinessId AND p.[AdministrationGroupId] = @AdministrationGroupId AND p.ProviderAffiliationId IN @ProviderAffiliationIds
                ORDER BY p.[ProviderAffiliationId], p.[City] 
            ";
        }
