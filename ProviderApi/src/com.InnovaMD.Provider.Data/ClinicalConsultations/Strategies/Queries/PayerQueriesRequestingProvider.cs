...
{
    internal static class PayerQueriesRequestingProvider
    {
        public static string SearchRequestingProvider(RequestingProviderSearchCriteria criteria, bool count = false)
        {
            if (count)
            {
                return @$"
                    ...
                    WHERE pnl.[LineOfBusinessId] = @LineOfBusinessId AND pnl.HealthPlanId = @HealthPlanId AND p.RenderingProviderEntityTypeId = {(int)ProviderEntityTypes.Individual}
        ...
        public static string GetProvidersLocations()
        {
            return @$"
                ...
                FROM  [dbo].[ProviderDirectory] p
                      JOIN [dbo].[ProductNetworkLineOfBusiness] pnl ON pnl.[NetworkId] = p.[NetworkId]
                WHERE pnl.[LineOfBusinessId] = @LineOfBusinessId AND pnl.HealthPlanId = @HealthPlanId AND p.ProviderAffiliationId IN @ProviderAffiliationIds
