...
{
    public class PayerRequestingProviderRepository : RepositoryBase, IPayerRequestingProviderRepository
    {
        public PayerRequestingProviderRepository(ConnectionStringOptions connectionStringOptions) : base(connectionStringOptions)
        {
        }

        public (IEnumerable<RequestingProvider> providers, int count) SearchRequestingProviders(RequestingProviderSearchCriteria criteria, int lineOfBusinessId, int healthPlanId)
        {
            using (var conn = new SqlConnection(connectionStringOptions.ClinicalConsultation))
            {
                using (var dao = new Dao(conn))
                {
                    ...
                    var count = dao.ExecuteScalar<int>(PayerQueriesRequestingProvider.SearchRequestingProvider(criteria, true), parameters);
                    var providers = new List<RequestingProvider>();

                    if (count > 0)
                    {
                        providers = dao.Find<RequestingProvider>(
                            PayerQueriesRequestingProvider.SearchRequestingProvider(criteria),
                            parameters: parameters).ToList();

                        if (providers != null && providers.Count > 0)
                        {
                            var reader = dao.FindMultiple(new string[] { PayerQueriesRequestingProvider.GetProvidersLocations(), PayerQueriesRequestingProvider.GetProvidersSpecialties() }, parameters: new {
                                LineOfBusinessId = lineOfBusinessId,
                                HealthPlanId = healthPlanId,
